...
await page.goto(base+(process.env.PUBLICATION_URL?'':'index.html'));assert.equal(await page.locator('.chapter').count(),30);assert.equal(await page.locator('.reader>h1').innerText(),'Cape Town.\nA closer look.');
...
await page.locator('.reportheader').getByRole('link',{name:'Original atlas',exact:true}).click();await page.waitForFunction(()=>document.querySelector('h1')?.textContent.includes('Where Cape Town can still build'));assert((await page.locator('body').innerText()).includes('Where Cape Town can still build'));
await page.getByRole('link',{name:'Report home',exact:true}).click();await page.waitForFunction(()=>document.querySelectorAll('.chapter').length===30);assert.equal(await page.locator('.chapter').count(),30);
await page.locator('.reportheader').getByRole('link',{name:'Downloads',exact:true}).click();await page.locator('.route').first().waitFor();assert.equal(await page.locator('.route').count(),4);assert.equal(await page.locator('.contents a').count(),18);
